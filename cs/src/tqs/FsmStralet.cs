...
namespace TQuant.Stralet
{
    ...
    {
        private FSMStraletBase stralet;

        private IStraletContext context;

        public FsmStraletWrap(FSMStraletBase stralet)
        {
        ...
        {
            context = sc;// new StraletContextImpl(sc);
            stralet.SetContext(context);
            stralet.Receive(FSMStraletBase.OnInit.Instance);
        }

        public override void OnFini()
        {
            stralet.Receive(FSMStraletBase.OnFini.Instance);
        }

        public override void OnQuote(MarketQuote quote)
        {
            stralet.Receive(new FSMStraletBase.OnQuote(quote));
        }
        public override void OnBar(String cycle, Bar bar)
        {
            stralet.Receive(new FSMStraletBase.OnBar(cycle, bar));
        }
        ...
        {
            stralet.Receive(new FSMStraletBase.OnOrder(order));
        }

        public override void OnOrderTrade(Trade trade)
        {
            stralet.Receive(new FSMStraletBase.OnTrade(trade));
        }
    ...
    public abstract class FSMStraletBase : IFsmStralet
    {
        private FsmStraletWrap stralet;

        protected FSMStraletBase()
        {
        ...
        public virtual bool Receive(object message) { return false; }
        ...
        internal void SetContext(IStraletContext ctx)
        {
            this.ctx = ctx;
        }
    ...
    public abstract class FsmStralet<TState, TData> : FSMStraletBase
    {
        ...
        public void SetTimer(string name, object msg, TimeSpan timeout, bool repeat = false)
        {
            ...
            if (name2timers.TryGetValue(name, out timer))
            {
                ...
                Context.KillTimer(this.Stralet, timer.Id);
            }
        ...
        {
            if (message is OnTimer)
            {
                var on_timer = message as OnTimer;
                Timer timer;
        ...
        private void Terminate(State<TState, TData> upcomingState)
        {
            if (_currentState.StopReason == null)
            {
                ...
                foreach (var t in id2timers)
                {
                    this.Context.KillTimer(this.Stralet, t.Value.Id);
                }
                id2timers.Clear();
                name2timers.Clear();
                ...
                if (this._stateTimout != null)
                {
                    this.Context.KillTimer(this.Stralet, this._stateTimout.Id);
                    this._stateTimout = null;
                }
